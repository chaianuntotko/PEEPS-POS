...
function enterApp(silent) {
  ...
  var isAdmin = String(ME.role || '').toLowerCase() === 'admin';
  $('me-name').textContent = ME.username;
  $('me-role').textContent = isAdmin ? 'ผู้ดูแลระบบ' : 'พนักงาน';
  $('me-initial').textContent = String(ME.username || '?').charAt(0);
  $('me-initial').hidden = isAdmin;
  $('me-avatar').hidden = !isAdmin;
...
function openMenuForm(item) {
  ...
  var catSelect = $('menu-category');
  /* เมนูเก่าอาจมีหมวดหมู่นอกเหนือรายการที่กำหนดไว้ — เพิ่มเป็นตัวเลือกชั่วคราวไม่ให้ค่าหาย */
  if (item && item.category && !catSelect.querySelector('option[value="' + item.category.replace(/"/g, '\\"') + '"]')) {
    catSelect.insertAdjacentHTML('beforeend', '<option value="' + esc(item.category) + '">' + esc(item.category) + '</option>');
  }
